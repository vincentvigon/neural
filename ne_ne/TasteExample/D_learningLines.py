...
from ne_ne.dataGen.dataLines.dataDealer import batch_of_lines_Y_is_orientation, batch_of_curve_Y_is_angle, \
    batch_of_curve_Y_is_width
...
class Model_fullyConv:
    ...
    def __init__(self,h_img:int,w_img:int,nbChannels:int,nbCategories,favoritism):
        ...
        self._accuracy_trivial=tf.reduce_mean(tf.cast(tf.equal(0, self._Y_cat), tf.float32))
...
def tasteClassif():
    ...
    if dataChoice==0:
        nbAngleCat = 3
        """il y a la catégorie 0 en plus qui correspond au fond"""
        nbCat = nbAngleCat + 1
        dataDealer=lambda batchSize: batch_of_lines_Y_is_orientation(nbAngleCat=nbAngleCat, batchSize=batchSize, img_size=28)
    elif dataChoice==1:
        nbAngleCat = 3
        """il y a la catégorie 0 en plus qui correspond au fond"""
        nbCat = nbAngleCat + 1
        dataDealer=lambda batchSize: batch_of_curve_Y_is_angle(nbAngleCat=nbAngleCat, batchSize=batchSize, img_size=28)
    else:
        """ 2 épaisseurs, et le background """
        nbCat=3
        dataDealer=lambda batchSize: batch_of_curve_Y_is_width(batchSize=batchSize,img_size=28)





    model = Model_fullyConv(28, 28, 1, nbCat,favoritism=None)
    model.verbose = True
    ...
    try:
        for step in range(nbStep):
            ...
            X_batch, Y_batch,_ = dataDealer(batchSize)
            ...
            if step%20==0:
                print("\nVALIDATION")
                X_val, Y_val ,_= dataDealer(batchSize)
    ...
    drawOne(Y_test,"Y",nbCat)


    draw_proba_Vs_cat=1
    if draw_proba_Vs_cat==0:
        for i in range(1,nbCat):
